...
class LoserAgent(sc2.BotAI):
    ...
    def __init__(self, is_logging = False, loggingName="", isMainAgent = False):
        super().__init__()
        ...
        if isMainAgent == True:
            ...
            if self.is_logging:
                ...
                if not os.path.exists("./logs"):
                    os.mkdir("./logs")
                self.log_file_name = "./logs/" + loggingName + strftime("%Y-%m-%d %H%M%S", localtime()) + ".log"
    ...
    def get_remaining_supply(self):
        ...
        return self.mainAgent.supply_left
    ...
    def log(self, data):
        ...
        if self.mainAgent.is_logging and os.path.getsize(self.log_file_name) < 1000000:
            self.mainAgent.log_file.write(data + "\n")
