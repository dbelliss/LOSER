...
class SafeRoachAgent(LoserAgent):
    ...
    async def basic_build(self, iteration):
        ...
        if self.mainAgent.base_build_order_complete is True:
                    ...
                    for d in range(4, 10):
                        if self.mainAgent.can_afford(HYDRALISKDEN):
                            err = await self.mainAgent.build(HYDRALISKDEN, near=self.mainAgent.units(
                                LAIR).ready.first.position.to2.towards(self.mainAgent.game_info.map_center, d))
                            if not err:
                                print("SUCCESSFULLY BUILT HYDRALISK DEN")
                                self.mainAgent.built_hd = True

            if larvae.amount > self.mainAgent.units(
                    HATCHERY).ready.amount * 4 and self.mainAgent.minerals > 700 and self.mainAgent.supply_left > 2:
            ...
            if self.mainAgent.minerals > 800 and (self.mainAgent.workers.amount > self.mainAgent.units(
                    HATCHERY).amount * 16 or self.mainAgent.workers.amount > 75):
            ...
            if (self.mainAgent.minerals > 500 and (
                    ...
                    (
                            self.mainAgent.minerals > 50 and self.mainAgent.extractors_built < 3) and not self.mainAgent.already_pending(
                EXTRACTOR):
            ...
            if self.mainAgent.supply_left < 4 and self.mainAgent.already_pending(
            ...
            if (self.mainAgent.workers.amount + self.mainAgent.already_pending(DRONE)) < (
                    self.mainAgent.units(HATCHERY).ready.amount * 16) and \
            ...
            if self.mainAgent.can_afford(HYDRALISK) and self.mainAgent.units(
                    HYDRALISK).amount + self.mainAgent.already_pending(HYDRALISK) < 10 and \
        ...
        if self.mainAgent.base_build_order_complete is False:
            ...
            if self.mainAgent.sporecrawlers_built >= 2 and self.mainAgent.built_rwarren is False and self.mainAgent.can_afford(
                ...
                for d in range(4, 10):
                    if self.mainAgent.can_afford(ROACHWARREN):
                        err = await self.mainAgent.build(ROACHWARREN, near=self.mainAgent.units(HATCHERY).find_by_tag(
            ...
            if self.mainAgent.units(ROACHWARREN).ready.exists and self.mainAgent.can_afford(
                    UPGRADETOLAIR_LAIR) and self.mainAgent.built_lair is False:
